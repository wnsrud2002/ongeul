...
테스트 119개 통과 (표준 unittest, 프레임워크 없음)
...
글자만 추려 넣는다(23MB 글꼴 → 400KB 부분집합). 글꼴이 본문 글자를 실제로 갖고 있는지
먼저 확인하고, 없으면 만들지 않고 이유를 말한다.

**PDF는 읽으라고 만드는 것이라 기계용 표기를 그대로 옮기지 않는다.**

- 검사 기록은 날것의 JSON이 아니라 `검사 항목 | 결과` 표와 상태 배지로 보여준다
- `<!-- 구역: ... -->` 같은 기계용 메모는 그리지 않고 옅은 선으로 대신한다
- 둘째 쪽부터 문서 이름을 머리글로 달고, 쪽번호는 `3 / 14` 로 전체를 함께 보여준다
- 제목이 쪽 끝에 혼자 남지 않고, 표가 쪽을 넘어가면 머리 행을 다시 그린다
- 제목·목록·표·인용·코드 블록을 그리고, 그림은 링크가 아니라 본문에 넣는다
- PDF 안에 "이것은 읽기용이고 보존을 보장하는 것은 `.md` 와 `.assets` 다"라고 적어 둔다
